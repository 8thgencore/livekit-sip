...
func classifyInviteError(err error) inviteFailure {
	...
	if sipStatus, ok := errors.AsType[*livekit.SIPStatus](err); ok {
		...
		case int(sip.StatusForbidden):
			res.status, res.term, res.reason = callDropped, stats.ServerError("forbidden"), livekit.DisconnectReason_SIP_TRUNK_FAILURE
		...
		case int(sip.StatusRequestTerminated):
			res.status, res.term, res.reason = callRejected, stats.ClientError("request-terminated"), livekit.DisconnectReason_USER_REJECTED
			res.reportErr = nil
		...
		case int(sip.StatusInternalServerError):
			res.status, res.term, res.reason = callDropped, stats.ServerError("internal-server-error"), livekit.DisconnectReason_SIP_TRUNK_FAILURE
		case int(sip.StatusBadGateway):
			res.status, res.term, res.reason = callDropped, stats.ServerError("bad-gateway"), livekit.DisconnectReason_SIP_TRUNK_FAILURE
		case int(sip.StatusServiceUnavailable):
			res.status, res.term, res.reason = callDropped, stats.ServerError("service-unavailable"), livekit.DisconnectReason_SIP_TRUNK_FAILURE
		case int(sip.StatusGlobalDecline):
			res.status, res.term, res.reason = callRejected, stats.ClientError("declined"), livekit.DisconnectReason_USER_REJECTED
			res.reportErr = nil
		default:
			switch {
			...
			case code >= 500 && code < 600:
				res.status, res.term, res.reason = callDropped, stats.ServerError("sip-5xx"), livekit.DisconnectReason_SIP_TRUNK_FAILURE
